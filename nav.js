...
$.ajax({
  ...
  success: function(res) {
      ...
      if(res.length === 0){
        ...
        cartProducts.forEach((product)=>{
            $('#d-cart-contents').append(` <div class="d-grid d-cart-item-desc" data-id=${product.id}>
  ...
  // for empty cart 
  $(document).on('click', ".d-AddBtn-cart", function (e) {
    e.stopPropagation()
    if (currentUser === null) {
      window.location.href = "login.html";
    } else {
      console.log('added to cart')
      // get id of product and make API call
  ...
  $(document).on('click', '.d-addCart', function(e){
    e.stopPropagation()
    if(currentUser !== null){
      console.log('added to cart')
      // usersItems.forEach((user) => {
